    ...
    Scaffold(modifier = Modifier.fillMaxSize() , topBar = {
        ...
        firestore.collection("users").whereEqualTo("id" ,user?.uid).get().addOnSuccessListener { it ->
            it.first().let {
                ...
                oldInfo.copy(name , phoneNumber , address)
            }
        }
        Column(modifier = Modifier
            .fillMaxSize()
            .padding(ip).padding(8.dp)) {
            Icon(imageVector = Icons.Default.AccountCircle , tint = Blue, modifier = Modifier.size(240.dp) , contentDescription = "user")
            ...
            } , enabled =name != oldInfo.name && phoneNumber != oldInfo.phone && address != oldInfo.address) {
                Text(text = "Add")
            }
